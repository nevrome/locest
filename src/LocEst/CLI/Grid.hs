...
type Ring    = [Point]
type Polygon = [Ring] -- exterior:holes (first element exterior, the ones potential holes))

-- GeoJSON parsing (same as before)
...
bbox polys =
  foldl' step (inf, inf, -inf, -inf) allPts
  where
    inf = 1/0
...
scanlineIntersections y ring =
  ...
  where
    edges = zip ring (tail (cycle ring))
    intersects y1 y2 =
      (y1 > y) /= (y2 > y)    -- standard half-open rule
...
emitPoints dx x0 intervals y =
  [ (x,y)
  | (a,b) <- intervals
  , let n0 = ceiling ((a - x0) / dx)
  , let n1 = floor   ((b - x0) / dx)
  , n <- [n0 .. n1]
